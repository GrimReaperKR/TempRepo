...
[CreateAssetMenu(fileName = "SO_CC_Knockback", menuName = "InGameEffect/CC/Knockback")]
public class SO_CC_Knockback : SOBase_UnitEffectEvent
{
    public override void OnInitialize(UnitBase _caster, UnitBase _victim, UNIT_EFFECT _index, float[] _value, bool _canRemove)
    {
        // 넉백은 탱커에게는 적용하지 않음
        if (_victim.UnitSetting.unitClass == UnitClass.Tank || _victim.IsStackPosition)
        {
            MgrObjectPool.Instance.ShowObj("tmpDmg", _victim.GetUnitCenterPos() + Vector3.up * 0.5f).GetComponent<DamageText>().SetDamageText(0.0f, _immunity: true, _unitEffectIndex: (int)_index);
            return;
        }
