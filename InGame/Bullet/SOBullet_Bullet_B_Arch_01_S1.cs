...
[CreateAssetMenu(fileName = "SOBullet_Bullet_B_Arch_01_S1", menuName = "Bullet/Bullet_B_Arch_01_S1")]
public class SOBullet_Bullet_B_Arch_01_S1 : SOBase_Bullet
{
    ...
    private class PersonalVariable : BulletPersonalVariableInstance
    {
        ...
        {
            listUnit.Clear();
            listUnit.AddRange(MgrBattleSystem.Instance.GetNearestEnemyUnitInEllipse(bulletComp.owner, bulletComp.transform.position, bulletComp.owner.GetUnitSkillFloatDataValue(0, "param.0"), 2, _isContainBlockedTarget: true));

            foreach(UnitBase unit in listUnit)
            {
                MgrInGameEvent.Instance.BroadcastDamageEvent(bulletComp.owner, unit, bulletComp.bulletAtk * bulletComp.owner.GetUnitSkillFloatDataValue(0, "param.1"), bulletComp.bulletCriRate, bulletComp.bulletCriDmg, 0);
                MgrObjectPool.Instance.ShowObj(bulletComp.bulletSetting.bulletHitToPlayerPrefabName, unit.GetUnitCenterPos());
            }

            SOBullet_Bullet_B_Arch_01_S1 comp = bulletComp.SoBullet as SOBullet_Bullet_B_Arch_01_S1;
            ...
            MgrSound.Instance.PlayOneShotSFX("SFX_B_Arch_01_s2_3", 1.0f);
        }
    }
}
